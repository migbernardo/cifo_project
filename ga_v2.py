...
from data import very_hard


class Solver:
    """ Base class of the genetic sudoku solver """

    def __init__(self, item: list, pop_size: int):
        """
        Initializes a set of randomly filled solutions considering the given puzzle.

        :param item: list of lists representation of the puzzle to solve where the non-given postions are marked with zeros
        :param pop_size: the number of solutions to generate for the initial population which can be evolved with the genetic algorithm (only even values are accepted)
        """
        ...
        # SAFEGUARD
        # check if array is a 9x9 grid that contains non-given positions (zero)
        ...
        # continue if requeriments are fulfilled
        else:
            # convert array into 81 digits representation arranged by 3x3 sub-grids
            self.rep = get_rep(self.arr)

            # store fixed and not fixed positions
            self.fixed = np.where(self.rep != 0)[0]
            self.not_fixed = np.where(self.rep == 0)[0]

            # initialize an empty population and fill it with random solutions
            self.pop = []
            for i in range(self.pop_size):
                rep = self.rep.copy()
                for index in self.not_fixed:
                    # replace not fixed positions with random numbers from 1 to 9
                    rep[index] = randint(1, 9)
                self.pop.append(rep)

            # initialize an empty list to store the fitness value of the generation's best individual
            self.fitness = []

            # convert arr into not flat representation and save its respective 3x3 sub-grid for inner swap mutation
            self.rep2 = get_rep2(self.arr)
            self.not_fixed_block = []
            for block in self.rep2:
                self.not_fixed_block.append(list(np.where(block == 0)[0]))

            # placeholder for the final solution
    ...
    def evolve(self,
               ...
               t_size: int = None,
               elitism: bool = False,
               global_optimum: int = None,
               iterations: int = None,
               export: bool = False,
               file_name: str = None):

        """
        Method to evolve the initial population generated by the Solver class using a Genetic Algorithm (GA).

        :param num_generations: number of generations to evolve the solutions
        :param selection: type of selection to perform ['tournament', 'fps', 'ranking']
        :param crossover: type of crossover to perform ['tpco', 'opco', 'fitness_co']
        :param co_p: crossover probability from 0.0 to 1.0
        :param mutation: type of mutation to perform ['opm', 'npm', 'swap', 'n_swap', 'inner_swap', 'rand_mut']
        :param mu_p: mutation probability from 0.0 to 1.0
        :param opt: type of optimization problem ['max', 'min']. 'max' tries to find solutions which maximize
                    the number of unique digits by row, column, and 3x3 sub-grid. 'min' does the opposite, minimizing
                    the number of non-unique digits by row, column, and 3x3 sub-grid
        :param fitness_type: pick which type of fitness to perform ['sum', 'avg']. Either the sum or average of numbers
                             (unique or non-unique) is considered by the algorithm
        :param n_mutations: specifies the number of mutations to perform by the 'npm' genetic operator. None by default.
                            Only accepts integer values
        :param num_swap: specifies the number of mutations to perform by the 'n_swap' genetic operator. None by default.
                         Only accepts integer values
        :param t_size: specifies the tournament size of the selection method. None by default. Only accepts integer values
        :param elitism: specifies if the worst individual, in terms of fitness, of each generation should be
                        replaced by the best individual of the previous population. False by default
        :param global_optimum: value at which the algorithm returns an acceptable solution (global optimum value is not mandatory).
                               None by default
        :param iterations: specifies for how long the 'fitness_co' should attempt to improve the average fitness of the parents.
                           None by default
        :param export: specifies whether to export the fitness values of the best individual of each generation to a .csv file.
                       False by default
        :param file_name: .csv file's name
        :return: Returns an evolved solution of a given sudoku puzzle, its fitness plot, and the values exported to .csv
        """
...
if __name__ == '__main__':
    puzzle = Solver(item=very_hard, pop_size=1000)

    puzzle.evolve(num_generations=50,
