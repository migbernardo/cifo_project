...
def tpco(p1, p2, not_fixed_index, shuffle):
    if shuffle:
        c1 = list(p1)
        c2 = list(p2)
        not_fixed_list = list(not_fixed_index)
        shuffled_list = sample(not_fixed_list, k=len(not_fixed_list))
        for original, shuffled in zip(not_fixed_list, shuffled_list):
            c1[original], c1[shuffled] = c1[shuffled], c1[original]
            c2[original], c2[shuffled] = c2[shuffled], c2[original]
        ...
        for original, shuffled in zip(not_fixed_list, shuffled_list):
            c1[shuffled], c1[original] = c1[original], c1[shuffled]
            c2[shuffled], c2[original] = c2[original], c2[shuffled]
    else:
        c1 = list(p1)
        c2 = list(p2)
        start_index = [9 * x for x in range(0, 9)]
...
if __name__ == '__main__':
    ...
    b1 = tpco(rep1, rep2, [0, 5, 2, 8], shuffle=True)[0]

    b2 = tpco(rep1, rep2, [0, 5, 2, 8], shuffle=True)[1]
